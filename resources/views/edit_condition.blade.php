...
try {
    ...
    $condition = $db_condition->getConditionById($id);                      //IDで商品状態取得    
...
<body>
    ...
    <div class="container col-9">
        <!-- <form action="./edit_action_condition.php" method="post"> -->
        <form action="edit_action_condition" method="post">
        @csrf 
            <div class="row mt-5">

                <input type="hidden" name="conditionId" id="conditionId" value="<?= $id ?>">
                ...
                <div class="mx-auto my-5">
                    ...
                    foreach ($conditionTypes as $conditionType) {
                        if ($conditionType['condition_id'] == $condition['condition_id']) {
                            ?>
                                <button type="submit" name="typeId" value="v<?= $conditionType['condition_id'] ?>" 
                                    onclick="type_func(this.value)" 
                    ...
                    ?>
                    <button type="submit" class="btn btn-lg btn-dark mx-3" id="cancelBtn" onclick="location.href='./index.php'">キャンセル</button>
                    <input type="hidden" name="maxConditionId" value="<?= $maxConditionId ?>">
                    <input type="hidden" name="conditionTypeId" id="conditionTypeId" value="<?= $maxConditionId ?>">
